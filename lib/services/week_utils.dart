...
class WeekUtils {
  ...
  /// Returns a human-readable date range label for the week that [date]
  /// falls into, e.g. "01-07 Sep 2026". Used for CSV export's Week column.
  static String weekDateRangeLabel(DateTime date) {
    final week = weekOfMonth(date);
    final startDay = (week - 1) * 7 + 1;
    final lastDayOfMonth = DateTime(date.year, date.month + 1, 0).day;
    final endDay = (week * 7) > lastDayOfMonth ? lastDayOfMonth : week * 7;

    final start = DateTime(date.year, date.month, startDay);
    final end = DateTime(date.year, date.month, endDay);

    final monthFmt = DateFormat('MMM yyyy');
    return '${start.day.toString().padLeft(2, '0')}-${end.day.toString().padLeft(2, '0')} ${monthFmt.format(end)}';
  }
}
