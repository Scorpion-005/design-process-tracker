...
class ExportService {
      ...
      for (int wIdx = 0; wIdx < mondaysInSection.length; wIdx++) {
        ...
        for (final day in dateOrder) {
          ...
          for (final d in dayDesigns) {
            ...
            _setCell(sheet, 2, row, d.customerName ?? '', colMaxLen: colMaxLen);
            _setCell(sheet, 3, row, 1, colMaxLen: colMaxLen);
            _setCell(sheet, 4, row, noFinish, colMaxLen: colMaxLen);
            _setCell(sheet, 5, row, noPending, colMaxLen: colMaxLen);
            _setCell(sheet, 6, row, d.rpNo ?? '', colMaxLen: colMaxLen);
            _setCell(
                sheet,
                7,
                row,
            ...
            _setCell(sheet, 8, row, d.designLeadTimeDays,
                background: _leadTimeHighlight, colMaxLen: colMaxLen);
            _setEmptyableCell(sheet, 9, row, d.cadApprovedDate, colMaxLen);
            _setEmptyableCell(sheet, 10, row, d.strikeOffDate, colMaxLen);
            _setCell(sheet, 11, row, d.sOffLeadTimeDays,
                background: _leadTimeHighlight, colMaxLen: colMaxLen);
            _setEmptyableCell(sheet, 12, row, d.rotaryScreenDate, colMaxLen);
  ...
  static void _setEmptyableCell(
      Sheet sheet, int col, int row, DateTime? date, Map<int, int> colMaxLen) {
    if (date == null) {
      _setCell(sheet, col, row, '-', fontColor: _redFont, colMaxLen: colMaxLen);
    } else {
      _setCell(sheet, col, row, _fmt.format(date), colMaxLen: colMaxLen);
    }
  }
